...
if(isset($_POST['submit']))
  {
    ...
    $sid = $_POST['state'];
    $query1=mysqli_query($con,"select * from tblstate where ID=$sid");
    $rw1=mysqli_fetch_array($query1);
    $statename=$rw1['StateName'];
    $cid = $_POST['city'];
    $query2=mysqli_query($con,"select * from tblcity where ID=$cid");
    $rw2=mysqli_fetch_array($query2);
    $cityname=$rw2['City'];
    $pid = $_POST['pg'];
    $query3=mysqli_query($con,"select * from tblhostels where id=$pid");
    $rw3=mysqli_fetch_array($query3);
    $pgname=$rw3['name'];
...
$fitemarray=implode(",",$fitem);
    $query=mysqli_query($con, "insert into tblfood(DonorID,foodId,StateName, CityName, pgName, FoodItems, Description, PickupDate,PickupAddress,ContactPerson,CPMobNumber,Image) value('$donorid','$fid','$statename','$cityname', '$pgname','$fitemarray','$description','$pdate','$padd','$contactperson','$cpmobnum','$foodpic')");
              ...
              <option value="<?php echo $row['ID'];?>"><?php echo $row['StateName'];?></option>
                  <?php } ?> 
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            <select class="form-control m-bot15" name="city" id="city" onChange="getPg(this.value);" required="true">
                    ...
                    <div class="form-group">
                        <label class="col-sm-3 control-label col-lg-3" for="inputSuccess">Choose PG</label>
                        <div class="col-lg-6">
                            <select class="form-control m-bot15" name="pg" id="pg" required="true">
